...
import { Calendar, Users, CheckCircle, Edit, Play, Trash2, Settings } from 'lucide-react'
...
export function ActivityCard({
  ...
  return (
    <Card
      ...
      className={`glass-effect border-slate-700/50 card-hover cursor-pointer transition-transform hover:scale-[1.02] ${!isUpcoming ? 'opacity-60' : ''
        }`}>
      <CardHeader>
          ...
          <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
            {canManage && (
              <Button
                ...
                title="Paramètres de l'activité (Shotcaller/Admin)"
              >
            ...
            {canRegisterOrEdit && (
              <>
                {!isRegistered ? (
                  ...
                  <Link href={`/activities/${activity.id}/register`}>
                    <Button size="sm" variant="outline" className="border-slate-700 text-purple-300 hover:text-purple-200">
